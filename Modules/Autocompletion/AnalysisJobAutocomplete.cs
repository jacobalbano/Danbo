...
internal class AnalysisJobAutocomplete : AutocompleteHandler
{
    public override Task<AutocompletionResult> GenerateSuggestionsAsync(IInteractionContext context, IAutocompleteInteraction autocompleteInteraction, IParameterInfo parameter, IServiceProvider services)
    {
        ...
        var result = analysisApi.GetJobs()
            ...
            .Select(x => new AutocompleteResult(x.Description, x.KeyGuid))
            .Take(25);
    ...
    {
        var term = $"{job.State.ToEmoji()} Job started {job.FirstStarted.InZone(tz).ToString("uuuu-MM-dd H:mm", null)}";

        int rank = search.Count(x => term.Contains(x));
        return new Rank(rank, term, job.Key.ToString());
    }

    private record struct Rank(int Count, string Description, string KeyGuid);
}
